...
VCF_DIR="${PROJECT_DIR}/Code/10x_multiomics/Sadeghi/Demultiplexing/liftover/vcf_files"
FASTA="/hg38/genome.fa"
VCF="/vcf_files/${pool}.merged.vcf.gz"

# Check if the BAM files exist
bam_gex="${BAM_DIR}/${pool}/outs/gex_possorted_bam.bam"
bam_atac="${BAM_DIR}/${pool}/outs/atac_possorted_bam.bam"

if [ ! -f "$bam_gex" ] || [ ! -f "$bam_atac" ]; then
  echo "BAM file not found for pool $pool. Exiting."
  exit 1
fi

# Check if the Souporcell and Vireo results already exist
souporcell_summary="${RESULTS_DIR_BASE}/RNA/genotype/souporcell/${pool}/souporcell_summary.tsv"
vireo_donor_ids="${RESULTS_DIR_BASE}/RNA/genotype/vireo/${pool}/donor_ids.tsv"

if [ -f "$souporcell_summary" ] && [ -f "$vireo_donor_ids" ]; then
  echo "Souporcell and Vireo results for $pool already exist. Skipping demultiplexing."
  exit 0
fi

# Set up singularity bind mounts
SINGULARITY_BINDS="--bind ${RESULTS_DIR_BASE}:/demuxafy --bind ${BAM_DIR}:/CellRanger_ARC --bind ${REF_DIR}:/hg38 --bind ${VCF_DIR}:/vcf_files"
...
run_souporcell() {
  ...
  local results_dir="${RESULTS_DIR_BASE}/${modality}/genotype"
  local bam
  local barcodes
  ...
  local out_dir="/genotype/${tool}/${pool}"
...
combine_results() {
  ...
  local out_dir="${results_dir}/combined/$pool"
