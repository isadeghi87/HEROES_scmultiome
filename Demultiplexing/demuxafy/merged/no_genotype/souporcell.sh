...
RESULTS_DIR="${PROJECT_DIR}/Result/10x_multiomics/Demultiplexing_results/demuxafy/merged/no_genotype"
...
BAM="/merge_dir/merged_sorted.bam"
...
SINGULARITY_BINDS="--bind ${RESULTS_DIR}:/no_genotype --bind ${BAR_DIR}:/CellRanger_ARC  --bind ${BAM_DIR}:/merge_dir --bind ${REF_DIR}:/hg38 --bind ${VCF_DIR}:/vcfs"
...
#Souporcell Summary
singularity exec $SINGULARITY_BINDS Demuxafy.sif bash souporcell_summary.sh $OUT_DIR/clusters.tsv > /omics/odcf/analysis/OE0290_projects/heroes-aya_pools/AG_Thongjuea/Result/10x_multiomics/Demultiplexing_results/demuxafy/merged/no_genotype/$tool/$pool/${tool}_summary.tsv
