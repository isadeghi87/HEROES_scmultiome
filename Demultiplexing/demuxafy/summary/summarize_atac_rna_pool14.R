...
v_rna = read.delim(file ='./Demultiplexing_results/demuxafy/RNA/genotype/vireo/pool14/donor_ids.tsv',
                 header = T)
colnames(v_rna)[1:2] = cols
v_rna = v_rna[,cols]
colnames(v_rna) = paste0('vireo_rna_',colnames(v_rna))

v_atac = read.delim(file ='./Demultiplexing_results/demuxafy/ATAC/no_genotype/vireo/pool14/donor_ids.tsv',
                   header = T)
colnames(v_atac)[1:2] = cols
v_atac = v_atac[,cols]
colnames(v_atac) = paste0('vireo_atac_',colnames(v_atac))

soup_rna = read.delim(file ='./Demultiplexing_results/demuxafy/RNA/no_genotype/souporcell/pool14/clusters.tsv',
                      header = T)
...
soup_atac = read.delim(file ='./Demultiplexing_results/demuxafy/ATAC/no_genotype/souporcell/pool14/clusters.tsv',
                      header = T)
...
merged$vireo_atac_assignment = case_when(merged$vireo_atac_assignment=='donor3'~"I074_025",
                                         merged$vireo_atac_assignment=='donor0'~'I007_072',
                                         TRUE ~ merged$vireo_atac_assignment)

merged$soup_atac_assignment = case_when(merged$soup_atac_assignment=='donor2'~"I074_025",
                                         merged$soup_atac_assignment=='donor1'~'I007_072',
                                         TRUE ~ merged$soup_atac_assignment)
merged$soup_rna_assignment = case_when(merged$soup_rna_assignment=='donor2'~"I074_025",
                                        merged$soup_rna_assignment=='donor0'~'I007_072',
                                        TRUE ~ merged$soup_rna_assignment)
...
write.table(merged_pass,'./Demultiplexing_results/demuxafy/summary/RNA_ATAC_pool14_majority.tsv',sep = '\t')
